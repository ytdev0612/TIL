        ...
        try:
            ...
            if x not in range(1, 10):
                ...
                if board[x-1] in ['●', '○']:  # 이미 누군가 둔 위치면 입력 불가
                    ...
                    board[x-1] = '●'
                    com_check_num.remove(x)  # 둔 위치 제거
                    break
        except ValueError:                      # 숫자가 아닌 다른 걸 입력했을 때 예외처리
            print('숫자로 입력해주세요.')

def player2_turn():                             # 컴퓨터가 랜덤으로 선택
    print('상대가 둘 차례입니다.')
    while True:
            x = random.choice(com_check_num)        # 1~9까지 랜덤으로 수를 선택, 이미 둔 수는 제거
            board[x - 1] = '○'
            com_check_num.remove(x)       # 둔 위치 제거
            print(com_check_num)
            break
...
while True:
    ...
    com_check_num = [1, 2, 3, 4, 5, 6, 7, 8, 9]  # 컴퓨터가 랜덤으로 숫자를 뽑을 때 해당 리스트에서 랜덤으로 선택
    cnt = 0  # 9번 반복 체크
    print_board()
